...
void addEdge(Graph graph,int idStart,int idDestination,char *start,char *destination,float distance){
    ...
    if(node){
        ...
        if(jrb_find_int(tree,idDestination) != NULL) return;
...
Dllist Dijkstra(Graph graph,int idStart,int idDestination){
    ...
    jrb_traverse(node,tree){
        ...
        jrb_insert_str(queue,a->name,new_jval_v(b));
        jrb_insert_str(parents,a->name,new_jval_s(strdup(inforStartAirport->name)));
    }


    float shortestCost;
    while(!emptyQueue(queue)){
        if(jrb_first(queue) == NULL) printf("NULL\n");
        // printf("Queue:\n");
        // jrb_traverse(node,queue){
        ...
        jrb_traverse(node,outcomesMinNodeEdges){
            ...
            if(nodeQueue == NULL && jrb_find_str(visited,airportNode->name) == NULL){
                ...
                if(t->cost < nodeQueueVal->cost){
                    ...
                    nodeInParentsTree->val = new_jval_s(strdup(minValue));
                }
            }
        ...
        jrb_insert_str(visited,minValue,new_jval_i(1));
    }
